...
- Daily summary: `pollen_daily` holds one row per city per UTC day (averages, peaks, species averages and peaks, reading count). Every daily reader (`/api/pollen?city=`, city pages, `/api/map-data`, `/api/city-type-matrix`, `/api/pollen-range?aggregate=day`, `/api/available-dates`) reads it instead of folding 24 hourly rows per city-day. Each ingest run recomputes the whole UTC days it wrote for the cities it wrote (`refreshPollenDaily` in `lib/pollen-daily.ts`); a refresh failure marks the run `partial`. After changing the aggregation, or on a database that predates the table, run `npm run db:backfill-pollen-daily` (optional `-- YYYY-MM-DD` start; month-sized statements; idempotent).
- Retention: the daily cron prunes `pollen_forecast_hourly` older than 7 days and `ambee_usage_logs` / `ingest_logs` older than 90 days after each run (`pruneOperationalData` in `lib/db.ts`). `pollen_readings_hourly`, `pollen_daily`, and `weather_daily` are kept indefinitely.
