...
| `aggregate` | ❌       | `none` or `day`; other values return `400`. `day` returns every whole UTC day that overlaps `[from, to)`, so a timestamp bound inside a day includes that entire day. |
